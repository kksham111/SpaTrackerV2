...
# Thread-local storage for video name
_thread_local = threading.local()

def set_video_name(video_name):
    """Set the current video name in thread-local storage."""
    _thread_local.video_name = video_name

def get_video_name():
    """Get the current video name from thread-local storage."""
    return getattr(_thread_local, 'video_name', None)
...
def weighted_procrustes_torch(X, Y, W=None, RT=None, video_name=None):
    # Get video_name from thread-local storage if not provided
    if video_name is None:
        video_name = getattr(_thread_local, 'video_name', None)
    """
    ...
    Args:
        ...
        video_name: (str, optional) video name for logging when orthogonalization is needed.
    ...
    det_R = torch.det(R)
    det_max_deviation = (det_R - 1).abs().max()
    if det_max_deviation < 1e-3:
        w2c[:, :, :3, :3] = R
    else:
        # Force orthogonalization using SVD to ensure det(R) = 1
        if video_name is not None:
            print(f"[WARNING] Force orthogonalization needed for video: {video_name} (det deviation: {det_max_deviation.item():.6f})")
        
        # Reshape R to (B*T, 3, 3) for batch SVD
        R_flat = R.view(B * T, 3, 3)
        U_svd, _, Vh_svd = torch.linalg.svd(R_flat)  # (B*T, 3, 3)
        R_corrected = U_svd @ Vh_svd  # (B*T, 3, 3)
        
        # Ensure det(R) = +1 (right-handed coordinate system)
        det = torch.linalg.det(R_corrected)  # (B*T,)
        neg_det_mask = det < 0
        U_flip = U_svd.clone()
        U_flip[neg_det_mask, :, -1] *= -1  # Flip last column for negative det
        R_corrected = U_flip @ Vh_svd
        
        # Reshape back to (B, T, 3, 3)
        R = R_corrected.view(B, T, 3, 3)
        w2c[:, :, :3, :3] = R
    w2c[:, :, :3, 3] = t
    try:
...
def key_fr_wprocrustes(cam_pts, graph_matrix, dyn_weight, vis_mask,slide_len=16, overlap=8, K=3, mode="keyframe"):
    ...
    if mode == "keyframe":
        ...
        for i in range(len(ky_fr_idx) - 1):
            ...
            relative_tfms = weighted_procrustes_torch(cam_ref, cam_win, weight, video_name=getattr(_thread_local, 'video_name', None))  # (B, W, 4, 4)
        ...
        for i in range(cam_pts_slide.shape[1]):
            ...
            if i == 0:
                c2w_traj_out[:, id_slide_i, :, :] = weighted_procrustes_torch(cam_pts_slide_i[:,:1], cam_pts_slide_i, weight_i, video_name=getattr(_thread_local, 'video_name', None))
            else:
                campts_update = torch.einsum("btij,btnj->btni", c2w_traj_out[:,id_slide_i][...,:3,:3], cam_pts_slide_i) + c2w_traj_out[:,id_slide_i][...,None,:3,3]
                c2w_traj_update = weighted_procrustes_torch(campts_update[:,:1], campts_update, weight_i, video_name=getattr(_thread_local, 'video_name', None))
